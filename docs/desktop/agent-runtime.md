...
| `session.prompt_complete` | managed ACP only: successful `session/prompt` `end_turn` with non-empty `agent_message_chunk` text. Local Service **seals the turn first** (stop managed process / cancel pending tool asks; turn busy→idle) then auto-calls `task.deliver` (summary=assistantText). Delivery must never publish while the same turn can still mutate the worktree. Empty/error/interrupt must **not** emit this event |
