...
| `session.failed` / `session.exited` (unintentional, pre-Delivery) / dead probe on remount | **Recoverable park (shared helper):** bound Task still `running`/`waiting` before any ready Delivery → `waiting(reason=external)` with durable `waitCode=session_unavailable` and stable English summary `SESSION_UNAVAILABLE_WAIT_SUMMARY`. **Keeps** occupation, worktree, report draft, pending TaskInputs, pending UserAsks, and audit facts. Session registry may be terminal/diagnostic; only Task is parked. Recovery is **explicit** `task.startSession` (or equivalent supported Service path) — no auto re-prompt / turn replay. Same-session duplicate terminals are idempotent; late events from a prior `sessionId` after rebind must not affect the new occupation. Multi independent Sessions terminating in the same tick each park only their own Task. Live runtime terminal projection and mount reconcile converge on this helper. **Invariant (diagnostic only):** once the bound Task is already `delivered` / `accepted` / `rejected` / collaboration-terminal, or reject-resume has parked `waiting(external)` with its own summary, or the session was intentionally sealed/stopped (`stopReason=user`), Session terminal is **diagnostic only** — update Session row / `lastError`, do **not** demote a published Delivery, do **not** re-park or `task.fail` those occupations, and do **not** cancel durable review-feedback U2A. Explicit `task.interrupt` / cancel remains terminal and releases/cleans per existing contract. Adapter may emit `session.failed` ("interrupted") after seal even when stop was intentional. |
...
7. **ACP native resume (`session/load` or `session/resume`):** when `canResume` and `resumeManagedSession` are implemented, `AgentRuntime.resumeSession` spawns a **new** bridge process and reuses the machine-local `resumeToken` as provider `sessionId` with the **same** recorded cwd. The adapter selects the wire transport (`session/load` by default; Claude uses `session/resume`). It must **not** call `session/new`. Load history notifications are diagnostics only (no auto-delivery); resume must not replay history. `task.startSession` may reuse an existing Tent `sessionId` after restart only when `probe.resumeCapable` is true **and** task lane cwd still matches the session row.
