...
| `session.failed` / dead probe unrecoverable | **Pre-delivery** only: `running`/`waiting` (non-park) → `task.fail` + occupation release. **Invariant:** once the bound Task is already `delivered` / `accepted` / `rejected` / collaboration-terminal, or reject-resume has parked `waiting(external)`, or the session was intentionally sealed/stopped (`stopReason=user`), Session terminal is **diagnostic only** — update Session row / `lastError`, do **not** demote a published Delivery, do **not** `task.fail` a parked reject-resume occupation, and do **not** cancel durable review-feedback U2A. Adapter may emit `session.failed` ("interrupted") after seal even when stop was intentional. |
