...
| Mutation (direct-core) | `dispatch`, `task-ack`, `task-cancel`, `report`, `complete`, `stamp`, `grant-readable`, `new-box`, `tag`, `untag`, `tag-new`, `tag-rm`, `fork`, `clean-temp`, `force-release`, `okf-sync` | **fail-loud** | allowed (migration window) |
