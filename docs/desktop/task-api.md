...
| `task.startSession` | authorized orchestration / user | Resolve **machine-local AgentProfile**, enforce **A2APolicy**, then service calls **internal** `AgentRuntimePort`. For agentProfile tasks, `profileId` must match the envelope assignee. **Authorized per-Task launch single-flight** (`workspaceId`+`taskPath`): every caller independently passes A2A/approval/policy/profile and task-state gates **before** joining any provider-launch flight (unauthorized callers never piggyback). After authorization, concurrent same-profile callers coalesce to one launch until the envelope `sessionId` bind completes; a different `profileId` while a launch is in flight fails with stable retryable `RPC_LIFECYCLE` (`task.startSession already in progress for this task`). If a usable bound Session already exists (non-terminal managed), return/reuse it without starting another provider. Late events from a prior `sessionId` remain harmless after rebind. Does **not** add a Task state; does **not** define force-fresh Session replacement as a public contract. |
