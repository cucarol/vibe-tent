...
| `task.sendInput` | **user only** | One-shot **U2A append** of `text` and/or `contextRefs` (stable entity ids) to a **running or waiting** managed task. Not chat; not conversation history; not profile mutation. Fail-loud if a pending UserAsk exists (use `userAsk.reply`). **RPC returns after durable accept only** (`accepted: true`, `input.status` typically `pending`; `continued: false`) — it does **not** wait for the provider Agent turn (avoids CLI false timeouts). Managed inject runs on a **per-task FIFO** background worker (`processing` → `delivered` \| `failed` with `lastError` \| `uncertain` when provider accepted but durable delivered mark failed). True inject failures are retained and retryable; **`uncertain` is at-most-once** (no auto/manual re-inject). Unrelated tasks stay concurrent. External agents poll + `taskInput.ack`. |
| `task.deliver` | assignee **or Local Service** (managed ACP auto-deliver) | Create/update delivery; enter `delivered` (or auto-integrate path per policy). Managed path: service seals the turn then calls the same lifecycle with `summary` = final assistant reply — never auto-accept beyond existing deliveryPolicy. **Public** `task.deliver` / `task.requestReview` fail-loud (`TURN_BUSY`) when the task is bound to a managed session with `runtime.probe(sessionId).turnBusy=true`; task stays `running` and no ready Delivery is published. Idle managed turns and external (no busy) sessions still allow manual deliver. |
| `task.requestReview` | assignee | Explicit review queue (used when `agent-decide` chooses upgrade). Same turnBusy gate as `task.deliver`. |
| `task.accept` | user; authorized orchestrator **≠ deliverer** | Integrate commits if any → `accepted`; clear occupation |
| `task.reject` | same as accept | Reject delivery; default resume rework. `resume: false` / `--no-resume` is terminal collaboration only (no session restore, no review U2A). When `resume: true`: (1) core reject → `running`, (2) persist lifecycle **review-feedback** TaskInput, (3) if the task already has a managed `sessionId`, restore/bind a live session (alive rebind or trackable new `ss-`; same role/profile restore rules as before) — restore failure parks `waiting(external)` and fails the RPC, never leave `running` with a stopped session, (4) **RPC returns after durable accept + restore only** (`accepted: true`, `enqueued: true` when managed, `input.status` typically `pending`, `continued: false`) — it does **not** wait for the provider Agent turn (same async accept contract as `task.sendInput`; avoids CLI/fetch headers timeout false-failures while Service finishes inject in the background). Managed inject of `## Review Feedback` runs on the **per-task FIFO** background worker (`processing` → `delivered` \| `failed` with `lastError` \| `uncertain` at-most-once). Failed rows stay retryable/poll-visible; already `processing`/`delivered`/`uncertain` skip re-inject (no double inject). Tasks without `sessionId` (external/manual) keep core-only rework + pending review-feedback for poll+ack. Delivery remains single-track (final report still via Delivery only). |
