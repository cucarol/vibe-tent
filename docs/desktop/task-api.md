...
| Restart | Terminal transitions observed by this feature enqueue a narrow pending marker under `temp/task-worktree-reclaim-pending.json`. `workspace.mount` retries **only those pending entries** — never infers opt-in from historical terminal envelopes or free-form worktree inventory. |
...
- `workspace.mount` recovery pass over **pending queue only** (restart-safe retry)
