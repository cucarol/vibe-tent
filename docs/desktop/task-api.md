...
| `task.deliver` | assignee **or Local Service** (managed ACP auto-deliver) | Create/update delivery; enter `delivered` (or auto-integrate path per policy). Managed path: service seals the turn then calls the same lifecycle with `summary` = final assistant reply — never auto-accept beyond existing deliveryPolicy. **Public** `task.deliver` / `task.requestReview` fail-loud (`TURN_BUSY`) when the task is bound to a managed session with `runtime.probe(sessionId).turnBusy=true`; task stays `running` and no ready Delivery is published. Idle managed turns and external (no busy) sessions still allow manual deliver. **Shared authority with managed auto-deliver:** any TaskInput on this task still `pending`, `processing`, or `failed` (retryable) fails-loud with stable code `PENDING_TASK_INPUT` (structured `inputIds` / `statuses` / `firstInputId` / `firstStatus` for UI invalidation); task stays non-terminal and **no** ready `dl-` is created. `uncertain` is at-most-once and does **not** block (store safety; no re-inject). Terminal `delivered` / `consumed` / `cancelled` rows do not block. Managed turn seal **must not** silently cancel open blocker rows to clear the gate. |
