...
async function readMutationLockRecord(lockPath) {
  try {
    ...
    if (isNotFound2(error) || error instanceof SyntaxError) return null;
    return null;
  }
}
...
${basePrompt}`;
    ...
    const message2 = rebuildErr instanceof Error ? rebuildErr.message : String(rebuildErr);
    return {
      continued: false,
      error: message2
    };
...
async function taskRejectRpc(ctx, p) {
  ...
  try {
    ...
    if (restored.session.contextRestored === false && restoredSessionId !== boundSessionId) {
    ...
    fallbackOrphanSessionId = void 0;
  ...
  if (rec?.lastTaskId) {
    ...
    for (const info of mountInfos) {
      ...
      if (ev.type === "session.waiting_user" && task.state === "running") {
        ...
        if (shouldSkipTaskFailOnSessionTerminal({
          sessionId: ev.sessionId,
          eventType: ev.type,
          stopReason: rec?.stopReason,
          task
...
function shouldSkipTaskFailOnSessionTerminal(input) {
  if (input.stopReason === "user") return true;
  if (input.eventType === "session.exited" && isManagedAutoDeliverSealing(input.sessionId, input.task.path, input.task.id)) {
    return true;
  }
  if (input.eventType === "session.failed" && isManagedAutoDeliverSealing(input.sessionId, input.task.path, input.task.id)) {
    return true;
  }
  if (rejectResumeNativeInFlight.has(input.sessionId)) return true;
  if (isRejectResumeParkedWait(input.task)) return true;
...
function buildRejectResumeRecoveryOrientation(task, roots, opts) {
  ...
  lines.push(
    opts.nativeResumeFailed ? "restorePath: native-resume-failed-fallback" : "restorePath: not-resume-capable-new-session"
  );
...
async function rebuildRejectResumeRecoveryOrientation(ctx, item) {
  if (normalizeTaskInputKind(item.kind) !== "review-feedback") return void 0;
  ...
  try {
    ...
    const message2 = err instanceof Error ? err.message : String(err);
    throw new Error(
      `reject-resume recovery orientation required but session registry read failed for ${sessionId}: ${message2}`
    );
  }
  if (!rec) {
    throw new Error(
  ...
  if (rec.contextRestored !== false) return void 0;
  const mount = ctx.host.get(item.workspaceId);
  if (!mount) {
    throw new Error(
  ...
  try {
    ...
    const message2 = err instanceof Error ? err.message : String(err);
    throw new Error(
      `reject-resume recovery orientation required (contextRestored=false) but task load failed for ${item.taskPath}: ${message2}`
    );
  }
  const workspaceLane = {
    workspace: task.workspace || mount.workspaceRoot,
...
async function stopOrphanRejectResumeSession(ctx, sessionId) {
  try {
    await ctx.runtime.stopSession(sessionId, "interrupt");
  } catch {
  }
...
async function restoreManagedSessionAfterRejectResume(ctx, input) {
  ...
  const restoreReason = nativeResumeError ? "task.reject.resume.native-fallback" : "task.reject.resume.new-session";
  ...
  try {
    ...
    if (nativeResumeError) {
      const message2 = err instanceof Error ? err.message : String(err);
      throw new Error(
        `native resume failed (${nativeResumeError}); independent recovery session also failed: ${message2}`
      );
    }
    throw err;
  }
...
function isOptionalString5(value) {
  return value === void 0 || typeof value === "string";
}
function isValidDate7(value) {
  return Number.isFinite(Date.parse(value));
}
...
function parseDraft(value) {
  ...
  if (!isRequiredString6(id) || !isRequiredString6(workspaceId) || !isRequiredString6(taskPath) || !isRequiredString6(sessionId) || !isRequiredString6(assistantText) || !isRequiredString6(createdAt) || !isRequiredString6(updatedAt) || !isValidDate7(createdAt) || !isValidDate7(updatedAt) || !isOptionalString5(taskId) || !isOptionalString5(lastError)) {
    return null;
  }
  ...
  return {
    ...
    workspaceId,
    taskPath,
    ...taskId !== void 0 && taskId.trim() ? { taskId: taskId.trim() } : {},
    sessionId,
    assistantText: text3,
    createdAt,
    updatedAt,
    ...typeof lastError === "string" && lastError.trim() ? { lastError: lastError.trim() } : {},
    attemptCount: attempts
  };
}
function makeManagedDeliveryReportDraftId(rand = Math.random) {
  const alphabet = "0123456789abcdefghjkmnpqrstvwxyz";
  let s = "mrd-";
  for (let i = 0; i < 10; i++) s += alphabet[Math.floor(rand() * alphabet.length)];
  return s;
}
var ManagedDeliveryReportDraftStore = class {
  constructor(dataDir, options) {
   ...
   */
  async get(workspaceId, taskPath) {
    if (!workspaceId?.trim() || !taskPath?.trim()) {
      throw new Error(
    ...
    return item ? cloneDraft(item) : void 0;
  }
  /**
  ...
  async list(workspaceId) {
    await this.ensureLoaded();
    return [...this.items.values()].filter((i) => !workspaceId || i.workspaceId === workspaceId).map(cloneDraft);
  }
  /**
   ...
   */
  async markFailed(workspaceId, taskPath, error) {
    if (this.closed) throw new Error("ManagedDeliveryReportDraft store is closed");
    if (!workspaceId?.trim() || !taskPath?.trim()) {
      throw new Error(
...
async function acquireServiceDataDirLease(dataDir, options = {}) {
  ...
  const instanceId = options.makeInstanceId?.() ?? randomUUID2();
    ...
    for (; ; ) {
      try {
        ...
        try {
          ...
          if (hasCode2(error2, "ENOENT")) continue;
          throw error2;
        }
      }
    }
...
async function readLeaseRecord(file) {
  try {
    ...
    if (hasCode2(error, "ENOENT")) return null;
