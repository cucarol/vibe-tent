...
2. 进入或恢复 Tent 会话时可运行 `tent status` 快速定向：看待裁提案、待 claim task、认领态，以及 Tent / workspace 两个路径。
...
5. 接任务后的第一步运行 `tent task claim <taskPath>`（经 Local Service RPC）。然后读取 envelope 指向的 manifest 与 claimed box；box 正文才是任务定义，envelope 只是不可变指针。复制 relay prompt 不是消费事件，只有 `task claim` 会把任务改成 `taken` / `running`。
...
11. 协作命令：
   ...
   - `tent task dispatch` / Desktop 派活：生成该 role 的 pending task envelope。user prompt 必填；真正认领发生在目标 agent 执行 `tent task claim` 时。
...
12. 收尾时在聊天里报告：改了什么、还剩什么、跑了什么测试、workspace commit hash。报告只写你**实际验证过**的事实——测试贴运行结果，修复贴复现前后对比；命令打了 ✓ 不等于结果发生了，关键动作要回读状态确认。交付用 `tent task deliver <taskPath> --summary <text> [--commits sha,sha]`（Local Service RPC）。

## 编排者手册

编排不是另一个 A2A 专属 skill；仍然使用 tent-role。

标准链路是：dispatch -> spawn/唤醒 -> claim -> deliver -> review -> accept。

- dispatch：写 manifest 和 pending task envelope，不写 owner/status。派活不要求你对目标 box 有 readable 或 writable——claim 权独立于读写权，唯一的门是占用拓扑：目标及其祖先、子孙没有 owner，也没有 pending task envelope，且不是归档/失效子树。编排 role 可以把任何无占用冲突的框派给别的 role；manifest 的写权是为接活 role 生成的，与派活者无关。
...
- claim：目标 agent 执行 `tent task claim <taskPath>`（Service RPC）后，envelope 变为 taken/running，并把目标 box owner 设为该 role、status 设为 doing。
...
> Legacy 直写 core 的 `tent dispatch` / `tent task-ack` / `tent report` 仍可用于离线测试，**不要**与 Desktop 共置时使用。
