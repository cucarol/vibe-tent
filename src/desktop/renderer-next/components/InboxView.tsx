...
export type InboxViewProps = { view: CollaborationSurfaceView; nodes: readonly WorkbenchNodeView[]; projection: "loading" | "fresh" | "stale" | "unresolved" | "error" | "unmounted"; onSelectNode: (nodeId: string) => void; onOpenDecision?: (nodeId: string) => void };
...
export function InboxView({ view, nodes, projection, onSelectNode, onOpenDecision }: InboxViewProps) {
  ...
  return <div className="tn-inbox-view" data-state={view.status} aria-busy={view.status === "loading" || view.status === "refreshing" || undefined}>
    ...
    {view.snapshot && items.length ? <ul className="tn-inbox-list" aria-label="待处理事项">{items.map((item) => {
      ...
      return <li key={item.kind === "result" ? item.resultId : item.requestId} className="tn-inbox-row-wrap">{node ? <button type="button" className="tn-inbox-row" data-actionable="true" onClick={() => item.kind === "decision" && onOpenDecision ? onOpenDecision(node.nodeId) : onSelectNode(node.nodeId)}>{content}</button> : <div className="tn-inbox-row" data-actionable="false" aria-disabled="true">{content}</div>}</li>;
    })}</ul> : null}
  </div>;
}
