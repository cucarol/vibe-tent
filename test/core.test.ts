...
test("completeClaim:默认只级联直属 output 记录", async () => {
  const dir = await makeTent();
  const fsa = new NodeFs(dir);
  const env = { fs: fsa, clock: { now: () => "t" }, tentName: "x" };
  const parent = "goal/挖新alpha/写表达式";
  await writeBoxNote(dir, `${parent}/交付记录`, "id: bx-delivery\ntype: output");
  await writeBoxNote(dir, `${parent}/参考交付`, "id: bx-reference\ntype: output-reference");
  await writeBoxNote(dir, `${parent}/后续工作`, "id: bx-followup\ntype: prompt");
  await writeBoxNote(
    dir,
    `${parent}/后续工作/嵌套产出`,
    "id: bx-nested-output\ntype: output",
  ...
  await completeClaim(env as any, "bx-g2");

  const after = await loadTent(fsa);
  assert.equal(after.byId.get("bx-g2")!.fm.status, "done");
  assert.equal(after.byId.get("bx-g2")!.fm.owner, undefined);
  assert.equal(after.byId.get("bx-delivery")!.fm.status, "done");
  assert.equal(after.byId.get("bx-reference")!.fm.status, "done");
  assert.equal(after.byId.get("bx-followup")!.fm.status, undefined);
  assert.equal(after.byId.get("bx-nested-output")!.fm.status, undefined);
});

test("completeClaim:cascadeOutputs=false 保留直属 output 状态", async () => {
  const dir = await makeTent();
  const fsa = new NodeFs(dir);
  const env = { fs: fsa, clock: { now: () => "t" }, tentName: "x" };
  await writeBoxNote(
    dir,
    "goal/挖新alpha/写表达式/交付记录",
    "id: bx-delivery\ntype: output",
  );
  const { completeClaim } = await import("../src/core/ops.js");

  await completeClaim(env as any, "bx-g2", { cascadeOutputs: false } as any);

  const after = await loadTent(fsa);
  assert.equal(after.byId.get("bx-g2")!.fm.status, "done");
  assert.equal(after.byId.get("bx-delivery")!.fm.status, undefined);
});

test("completeClaim:直属 output 被认领时整次拒绝", async () => {
  const dir = await makeTent();
  const fsa = new NodeFs(dir);
  const env = { fs: fsa, clock: { now: () => "t" }, tentName: "x" };
  await writeBoxNote(
  ...
  await assert.rejects(
    () => completeClaim(env as any, "bx-g2"),
    /bx-delivery.*reviewer/,
  );

  const after = await loadTent(fsa);
  assert.equal(after.byId.get("bx-g2")!.fm.status, "doing");
  assert.equal(after.byId.get("bx-g2")!.fm.owner, "executor");
  assert.equal(after.byId.get("bx-delivery")!.fm.status, "doing");
  assert.equal(after.byId.get("bx-delivery")!.fm.owner, "reviewer");
});

test("completeClaim:跳过归档与失效 output 记录", async () => {
  const dir = await makeTent();
  const fsa = new NodeFs(dir);
  const env = { fs: fsa, clock: { now: () => "t" }, tentName: "x" };
  const parent = "goal/挖新alpha/写表达式";
  await writeBoxNote(
    dir,
    `${parent}/归档交付`,
    "id: bx-archived-output\ntype: output\narchived: true",
  );
  await writeBoxNote(dir, `${parent}/重复交付 A`, "id: bx-duplicate-output\ntype: output");
  await writeBoxNote(dir, `${parent}/重复交付 B`, "id: bx-duplicate-output\ntype: output");
  const { completeClaim } = await import("../src/core/ops.js");
  ...
  const archived = parseFrontmatter(
    await fs.readFile(
      path.join(dir, ...`${parent}/归档交付/归档交付.md`.split("/")),
      "utf8",
    ),
  );
  const duplicateA = parseFrontmatter(
    await fs.readFile(
      path.join(dir, ...`${parent}/重复交付 A/重复交付 A.md`.split("/")),
      "utf8",
    ),
  );
  assert.equal(archived.data.status, undefined);
  assert.equal(duplicateA.data.status, undefined);
});
...
async function writeBoxNote(dir: string, relativePath: string, frontmatter: string): Promise<void> {
